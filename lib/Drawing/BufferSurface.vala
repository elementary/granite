...
namespace Granite.Drawing {
    ...
    public class BufferSurface : GLib.Object {
        ...
        void exponential_blur_columns (
            ...
            for (var columnIndex = start_col; columnIndex < end_col; columnIndex++) {
                // blur columns
                uint8 *column = pixels + columnIndex * 4;

                var z_a = column[0] << PARAM_PRECISION;
                var z_r = column[1] << PARAM_PRECISION;
                var z_g = column[2] << PARAM_PRECISION;
                var z_b = column[3] << PARAM_PRECISION;

                // Top to Bottom
                for (var index = width * (start_y + 1); index < (end_y - 1) * width; index += width)
                    exponential_blur_inner (&column[index * 4], ref z_a, ref z_r, ref z_g, ref z_b, alpha);
        ...
        void exponential_blur_rows (
            ...
            for (var rowIndex = start_row; rowIndex < end_row; rowIndex++) {
                // Get a pointer to our current row
                uint8* row = pixels + rowIndex * width * 4;

                var z_a = row[start_x + 0] << PARAM_PRECISION;
                var z_r = row[start_x + 1] << PARAM_PRECISION;
                var z_g = row[start_x + 2] << PARAM_PRECISION;
                var z_b = row[start_x + 3] << PARAM_PRECISION;

                // Left to Right
                for (var index = start_x + 1; index < end_x; index++)
                    exponential_blur_inner (&row[index * 4], ref z_a, ref z_r, ref z_g, ref z_b, alpha);
        ...
        private static inline void exponential_blur_inner (
            ...
            pixel[0] = (uint8) (z_a >> PARAM_PRECISION);
            pixel[1] = (uint8) (z_r >> PARAM_PRECISION);
            pixel[2] = (uint8) (z_g >> PARAM_PRECISION);
            pixel[3] = (uint8) (z_b >> PARAM_PRECISION);
        }
        ...
        static double[] build_gaussian_kernel (int gausswidth) requires (gausswidth % 2 == 1) {
            ...
            for (var i = 0; i < gausswidth / 2 + 1; i++)
                kernel[gausswidth - i - 1] =
                    kernel[i] =
                        Math.pow (Math.sin (((i + 1) * (Math.PI / 2) - mean) / range), 2) * sd;
